...
import kotlinx.android.synthetic.main.list_item_board_event_view_pager.view.*

class BoardEventPagerAdapter(
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        val view: View = inflater.inflate(R.layout.list_item_board_event_view_pager, null)

        val imageView = view.list_item_board_event_view_pager_test_image_view
        imageView.clipToOutline = true
        if (null != mImageList) {
            imageView.setImageResource(mImageList[position])
        }

        val simpleDraweeView = view.list_item_board_event_view_pager_image_view
        if (null != simpleDraweeView) {
            imageView!!.clipToOutline = true
            simpleDraweeView.setImageURI(Uri.parse(NetworkProvider.DEFAULT_IMAGE_DOMAIN + mBannerEventListData[position].getBannerImageName()))
        }
